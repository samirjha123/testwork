...
`pinning_version` and `version` attributes are now available for `rpm` package. The td-agent repository now use `reprepro` for building Deb repository. `reprepro` can not handle multiple versions of the same package.
